...
## 2. Uniform constructor return types

| # | Rule | Verify |
|---|------|--------|
| 2 | Every `TaskQueueFactory` constructor returns `Box<dyn TaskQueue>` | `grep -n "Box<dyn TaskQueue>" main/task-queue/saf/src/task_queue_factory.rs` shows every constructor's return type |
| 3 | Regression test exists proving constructors from different backends unify into one `Vec` | `cargo test -p task-queue-svc-saf --features inmemory,kafka test_kafka_and_in_memory_constructors_return_the_same_boxed_queue_type` passes |

## 3. No concrete backend type leaks through `saf`

| # | Rule | Verify |
|---|------|--------|
| 4 | `saf`'s own `lib.rs` never re-exports a concrete backend type | `grep -n "^pub use" main/task-queue/saf/src/lib.rs` shows only `TaskQueueFactory` |
